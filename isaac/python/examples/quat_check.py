...
class State_Check():
    def __init__(self):
        with open("../../IsaacGymEnvs/isaacgymenvs/cfg/task/Dynasoar.yaml", "r") as cfg:
            try:
        ...
        while not self.gym.query_viewer_has_closed(self.viewer):
            for evt in self.gym.query_viewer_action_events(self.viewer):
                ...
                if evt.action == "forward" and evt.value > 0:
                    ...
                    torques[:, 0, 0] = torch.tensor([0.01], device=self.device)
                    self.gym.apply_rigid_body_force_tensors(self.sim, gymtorch.unwrap_tensor(forces), gymtorch.unwrap_tensor(torques), gymapi.ENV_SPACE)
                    ...
                    torques[:, 0, 1] = torch.tensor([0.01], device=self.device)
                    self.gym.apply_rigid_body_force_tensors(self.sim, gymtorch.unwrap_tensor(forces), gymtorch.unwrap_tensor(torques), gymapi.ENV_SPACE)
                    ...
                    torques[:, 0, 2] = torch.tensor([0.01], device=self.device)
                    self.gym.apply_rigid_body_force_tensors(self.sim, gymtorch.unwrap_tensor(forces), gymtorch.unwrap_tensor(torques), gymapi.ENV_SPACE)
