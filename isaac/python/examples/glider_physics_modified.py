...
class LiftingLine:
    def __init__(self, glider_params, dt, debug_flags):
        ...
        self.Cd0 = 0.00
        self.frontal_area = 1.125e-2
        self.setup(self.glider_params)

        alpha_fields = ['Env']
        for i in range(20):
            alpha_fields.append('A{}'.format(i))
        self.alpha_log = CSVLogger('alphas.csv', fields=alpha_fields, test_name=debug_flags['log_name'])
    ...
    def compute_force_moment(self, Y, actions, initial_root_states):
        ...
        self.V_inf = Vinf
        self.alpha = alpha
        self.vec2 = alpha + alpha_mod - alpha_roll
        ...
        thrust_p = torch.unsqueeze(torch.tensor([-1,0,0],device=self.device),dim=0).repeat(self.M,1)*torch.unsqueeze(actions[:,5], dim=1)*1.0
        thrust_g = quat_rotate(glider_2_world_quat, thrust_p)
        generate_p = torch.unsqueeze(torch.tensor([1,0,0],device=self.device),dim=0).repeat(self.M,1)*torch.unsqueeze(actions[:,2], dim=1)*1.0
